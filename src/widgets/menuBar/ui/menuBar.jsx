...
export const Menubar = () => {
    ...
    const renderMultipleMenu = useCallback(() => {
        ...
        return menuConfig.map((item, index) => {
            ...
            if ((typeof item.roles === "object" && (user?.job.some(job => item.roles.includes(job.toLowerCase())) || (user?.role && item.roles.includes(user.role.toLowerCase())))) || (typeof item.roles === "boolean" && item.roles)) {
                return (
                    <NavLink
                        to={item.to}
                        key={index}
