...
import {
    getTaskCategoryList,
    getTaskLoading,
    getTaskNotificationLoading,
    getTaskNotificationsList,
    getTaskProfileLoading,
...
import { fetchTaskNotifications, fetchTasks, fetchTaskTags } from "../model/todoistThunk"
...
import { fetchEmployersDataWithoutPagination } from "entities/employer/model/slice/employersThunk.js"
...
const reducers = { employers: employersReducer }
...
const getDeadlineBackground = (task) => {
    if (task.status === "completed") return null

    const backgrounds = {
        red: "rgba(255, 0, 0, 0.12)",
        green: "rgba(0, 255, 0, 0.12)",
...
const NotificationCard = ({ data, onToggleRead, onViewTask }) => {
    const { id, message, mission, deadline, is_read, created_at } = data
    ...
    return (
        <div className={`${styles.card} ${is_read ? styles.read : ""}`}>
            ...
            <div className={styles.card__btn}>
                <button className={styles.viewBtn} onClick={() => onViewTask(mission)}>
                    Vazifani korish
                </button>
                <button className={styles.toggleBtn} onClick={() => onToggleRead(id, !is_read)}>
...
export const TodoistPage = () => {
        ...
        if (userId && activeTaskType) {
            if (activePage === "task") {
                const params = {
                    ...
                    created_at: filters.create,
                    deadline_after: filters.deadlineFrom,
                    deadline_before: filters.deadlineTo,
                    category: filters.category,
                    tags: filters.tags.length === 0 ? null : filters.tags.map(item => item.value)
                }

                const roleParams = {
                    myTasks: { executor: userId },
                    givenTask: { creator: userId },
                    viewTasks: { reviewer: userId }
                }

                dispatch(fetchTasks({ ...roleParams[activeTaskType], ...params }))
            } else {
                dispatch(fetchTaskNotifications({ role: activeNotificationType, user_id: userId }))
            }
        }
    }, [userId, activeTaskType, activePage, activeNotificationType, filters])
    ...
    useEffect(() => {
        if (tags) {
            setTagsList(tags.map(item => ({ value: item.id, label: item.name })))
        }
    }, [tags])
    ...
    const onToggleRead = (id, isRead) => {
        ...
        request(`${API_URL}Tasks/notifications/${id}/`, "PATCH", JSON.stringify({ is_read: isRead }), headers())
            ...
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
    ...
    const updateTaskInState = (res) => {
        if (selectedMultiTask && selectedMultiTask.children) {
            dispatch(editMultiTask({ ...res, parent: selectedMultiTask.id }))
    ...
    const onChangeRedirect = () => {
        const patch = { executor_ids: [+selectedRedirect] }
        ...
        request(`${API_URL}Tasks/missions/${selectedTask.id}/`, "PATCH", JSON.stringify(patch), headers())
            .then(res => {
                request(`${API_URL}Tasks/missions/${res.id}/`, "GET", null, headers())
                    .then(res => {
                        updateTaskInState(res)
                        dispatch(onAddAlertOptions({ status: true, type: "success", msg: "Vazifa yo'naltirildi" }))
                        setModalType(null)
    ...
    const handleCreateTask = () => {
        ...
        if (formData.recurring_type !== "custom") {
            repeat = { repeat_every: recurringTypes.filter(item => item.id === formData.recurring_type)[0]?.number }
        }
        ...
        request(`${API_URL}Tasks/missions/`, "POST", JSON.stringify(post), headers())
            .then(res => {
                ...
                dispatch(onAddAlertOptions({ status: true, type: "success", msg: "Vazifa yaratildi" }))
                setModalType(null)
        ...
        request(`${API_URL}Tasks/missions/${formData.id}/`, "PUT", JSON.stringify(patch), headers())
            .then(res => {
                request(`${API_URL}Tasks/missions/${res.id}/`, "GET", null, headers())
                    .then(res => {
                        updateTaskInState(res)
                        dispatch(onAddAlertOptions({ status: true, type: "success", msg: "Vazifa o`zgartirildi" }))
                        setModalType(null)
                    })
                    .catch(err => {
                        dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
    ...
    const handleDeleteTask = () => {
        ...
        request(`${API_URL}Tasks/missions/${selectedTask.id}/`, "DELETE", null, headers())
            .then(res => {
                if (selectedMultiTask && selectedMultiTask.children) {
                    dispatch(deleteMultiTask({ id: selectedTask.id, parent: selectedMultiTask.id }))
                    setSelectedMultiTask(setSelectedMultiTask(() => {
                        const filtered = {
                            ...selectedMultiTask.children.filter(item => item.id !== res.id)[0],
                            children: [
                                ...selectedMultiTask.children.filter(item => item.id !== res.id),
                ...
                dispatch(onAddAlertOptions({ status: true, type: "success", msg: `${selectedTask.title} - Vazifasi o'chirildi` }))
                setModalType(null)
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
        ...
        request(`${API_URL}Tasks/missions/${formData.id}/`, "PATCH", JSON.stringify({
            ...
            .then(res => {
                request(`${API_URL}Tasks/missions/${res.id}/`, "GET", null, headers())
                    .then(res => {
                        updateTaskInState(res)
                        dispatch(onAddAlertOptions({ status: true, type: "success", msg: "Vazifani statusi o`zgartirildi" }))
                        setModalType(null)
                    })
                    .catch(err => {
                        dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
    ...
    const handleCreateTag = () => {
        ...
        request(`${API_URL}Tasks/tags/`, "POST", JSON.stringify({ name: tagFormData }), headers())
            .then(res => {
                dispatch(addTag(res))
                dispatch(onAddAlertOptions({ status: true, type: "success", msg: "Teg yaratildi" }))
                setModalType(null)
                setTagFormData(null)
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Bu nomli teg bor yoki xatolik yuz berdi!" }))
    ...
    const handleEditTag = () => {
        ...
        request(`${API_URL}Tasks/tags/${selectedTag.id}/`, "PATCH", JSON.stringify({ name: tagFormData }), headers())
            .then(res => {
                ...
                dispatch(onAddAlertOptions({ status: true, type: "success", msg: `Tegni nomi "${res.name}"ga o'zgartirildi` }))
                setModalType(null)
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
    ...
    const handleDeleteTag = () => {
        ...
        request(`${API_URL}Tasks/tags/${selectedTag.id}/`, "DELETE", null, headers())
            .then(res => {
                ...
                dispatch(onAddAlertOptions({ status: true, type: "danger", msg: `"${selectedTag.name}" - tegi o'chirildi` }))
                setModalType(null)
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
    ...
    const handleCreateSubtask = () => {
        ...
        const post = { title: nestedFormData.title, order: nestedFormData.order, mission: selectedTask.id }

        request(`${API_URL}Tasks/subtasks/`, "POST", JSON.stringify(post), headers())
            .then(res => {
                dispatch(addSubTasks(res))
                setSelectedTask(prev => ({ ...prev, subtasks: [...prev.subtasks, res] }))
                setNestedModalType(null)
                dispatch(onAddAlertOptions({ status: true, type: "success", msg: "Sub-task qo'shildi" }))
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
    ...
    const handleEditSubtask = () => {
        dispatch(taskProfileLoading("subtasks"))
        const patch = { title: nestedFormData.title, order: nestedFormData.order }

        request(`${API_URL}Tasks/subtasks/${nestedFormData.id}/`, "PATCH", JSON.stringify(patch), headers())
            .then(res => {
                dispatch(editSubTasks(res))
                setSelectedTask({ ...selectedTask, subtasks: selectedTask.subtasks.map((s) => (s.id === res.id ? res : s)) })
                setNestedModalType(null)
                dispatch(onAddAlertOptions({ status: true, type: "success", msg: "Sub-task o'zgartirildi" }))
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
                dispatch(taskProfileLoadingStop())
    ...
    const handleDeleteSubtask = () => {
        ...
        request(`${API_URL}Tasks/subtasks/${nestedFormData.id}`, "DELETE", null, headers())
            .then(res => {
                ...
                setSelectedTask({ ...selectedTask, subtasks: selectedTask.subtasks.filter((s) => s.id !== nestedFormData.id) })
                setNestedModalType(null)
                dispatch(onAddAlertOptions({ status: true, type: "danger", msg: "Sub-task o'chirildi" }))
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
                dispatch(taskProfileLoadingStop())
    ...
    const handleCreateAttachment = () => {
        ...
        request(`${API_URL}Tasks/attachments/`, "POST", formDataImg, headerImg())
            .then(res => {
                ...
                setSelectedTask(prev => ({ ...prev, attachments: [...prev.attachments, res] }))
                setNestedModalType(null)
                formDataImg.delete("note")
                formDataImg.delete("file")
                formDataImg.delete("mission")
                dispatch(onAddAlertOptions({ status: true, type: "success", msg: "Qo'shimcha qo'shildi" }))
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
                dispatch(taskProfileLoadingStop())
    ...
    const handleEditAttachment = () => {
        ...
        request(`${API_URL}Tasks/attachments/${nestedFormData.id}/`, "PATCH", formDataImg, headerImg())
            .then(res => {
                ...
                setSelectedTask({ ...selectedTask, attachments: selectedTask.attachments.map((s) => (s.id === res.id ? res : s)) })
                setNestedModalType(null)
                formDataImg.delete("note")
                formDataImg.delete("file")
                dispatch(onAddAlertOptions({ status: true, type: "success", msg: "Qo'shimcha o'zgartirildi" }))
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
                dispatch(taskProfileLoadingStop())
    ...
    const handleDeleteAttachment = () => {
        ...
        request(`${API_URL}Tasks/attachments/${nestedFormData.id}`, "DELETE", null, headers())
            .then(res => {
                ...
                setSelectedTask({ ...selectedTask, attachments: selectedTask.attachments.filter((s) => s.id !== nestedFormData.id) })
                setNestedModalType(null)
                dispatch(onAddAlertOptions({ status: true, type: "danger", msg: "Qo'shimcha o'chirildi" }))
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
                dispatch(taskProfileLoadingStop())
    ...
    const handleCreateComment = () => {
        ...
        request(`${API_URL}Tasks/comments/`, "POST", formDataImg, headerImg())
            .then(res => {
                ...
                setSelectedTask(prev => ({ ...prev, comments: [...prev.comments, res] }))
                setNestedModalType(null)
                formDataImg.delete("text")
                formDataImg.delete("user")
                formDataImg.delete("attachment")
                formDataImg.delete("mission")
                dispatch(onAddAlertOptions({ status: true, type: "success", msg: "Koment qo'shildi" }))
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
                dispatch(taskProfileLoadingStop())
    ...
    const handleEditComment = () => {
        ...
        request(`${API_URL}Tasks/comments/${nestedFormData.id}/`, "PATCH", formDataImg, headerImg())
            .then(res => {
                ...
                setSelectedTask({ ...selectedTask, comments: selectedTask.comments.map((s) => (s.id === res.id ? res : s)) })
                setNestedModalType(null)
                formDataImg.delete("text")
                formDataImg.delete("attachment")
                dispatch(onAddAlertOptions({ status: true, type: "success", msg: "Koment o'zgartirildi" }))
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
                dispatch(taskProfileLoadingStop())
    ...
    const handleDeleteComment = () => {
        ...
        request(`${API_URL}Tasks/comments/${nestedFormData.id}`, "DELETE", null, headers())
            .then(res => {
                ...
                setSelectedTask({ ...selectedTask, comments: selectedTask.comments.filter((s) => s.id !== nestedFormData.id) })
                setNestedModalType(null)
                dispatch(onAddAlertOptions({ status: true, type: "danger", msg: "Koment o'chirildi" }))
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
                dispatch(taskProfileLoadingStop())
    ...
    const handleCreateProof = () => {
        ...
        request(`${API_URL}Tasks/proofs/`, "POST", formDataImg, headerImg())
            .then(res => {
                ...
                setSelectedTask(prev => ({ ...prev, proofs: [...prev.proofs, res] }))
                setNestedModalType(null)
                formDataImg.delete("comment")
                formDataImg.delete("file")
                formDataImg.delete("mission")
                dispatch(onAddAlertOptions({ status: true, type: "success", msg: "Proof qo'shildi" }))
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
                dispatch(taskProfileLoadingStop())
    ...
    const handleEditProof = () => {
        ...
        request(`${API_URL}Tasks/proofs/${nestedFormData.id}/`, "PATCH", formDataImg, headerImg())
            .then(res => {
                ...
                setSelectedTask({ ...selectedTask, proofs: selectedTask.proofs.map((s) => (s.id === res.id ? res : s)) })
                setNestedModalType(null)
                formDataImg.delete("comment")
                formDataImg.delete("file")
                dispatch(onAddAlertOptions({ status: true, type: "success", msg: "Proof o'zgartirildi" }))
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
                dispatch(taskProfileLoadingStop())
    ...
    const handleDeleteProof = () => {
        ...
        request(`${API_URL}Tasks/proofs/${nestedFormData.id}`, "DELETE", null, headers())
            .then(res => {
                ...
                setSelectedTask({ ...selectedTask, proofs: selectedTask.proofs.filter((s) => s.id !== nestedFormData.id) })
                setNestedModalType(null)
                dispatch(onAddAlertOptions({ status: true, type: "danger", msg: "Proof o'chirildi" }))
            })
            .catch(err => {
                dispatch(onAddAlertOptions({ status: true, type: "error", msg: "Xatolik yuz berdi!" }))
                dispatch(taskProfileLoadingStop())
    ...
    return (
        <DynamicModuleLoader reducers={reducers}>
            <div className={styles.container}>
                ...
                <div className={styles.content}>
                    ...
                    <section className={styles.section}>
                        <div className={styles.sectionHeader}>
                            <h1 className={styles.sectionTitle}>
                                ...
                                >
                                    Notifications
                                    {isHaveNot && <div className={styles.dott} />}
                                </span>
                            </h1>
                        ...
                        {/* Multi-task breadcrumb */}
                        {(selectedMultiTask && selectedMultiTask.children) && (
                            <div className={styles.multiTitle}>
                                <h2 className={styles.multiTitle__inner}>
                                    {selectedMultiTask.title} - Task Series
                                </h2>
                                <Button
                            ...
                            {activePage === "task" ? (
                                tasksLoading ? (
                                    ...
                                    <h1 className={styles.grid__title}>
                                        {activeTaskType === "myTasks" ? "Sizga vazifa berilmagan" :
                                            activeTaskType === "givenTask" ? "Siz vazifa bermagansiz" :
                                                "Tekshirishingiz kerak bolgan vazifa yo'q"}
                                    </h1>
                                ) : (
                                    sortTasks((selectedMultiTask && selectedMultiTask.children) || tasks).map((task) => (
                                        <div
                                            key={task.id}
                                            className={styles.card}
                                            style={getDeadlineBackground(task)}
                                        >
                                            ...
                                            <p className={styles.cardText}>
                                                <strong>
                                                    {(activeTaskType === "givenTask" && task?.children?.length > 1) ? "Executors:" : "Executor:"}
                                                </strong>
                                                {" "}
                                                {(activeTaskType === "givenTask" && task?.children?.length > 1)
                                            ...
                                            <div className={styles.cardActions}>
                                                {(userLevel !== 4 && (task?.children?.length === 1 || !task?.children)) && (
                                                    <button
                    ...
                    {userLevel < 2 && (
                        <section className={styles.section}>
                            ...
                            <div className={classNames(styles.tagsList, { [styles.loading]: tagsLoading })}>
                                {tagsLoading ? (
                                    <DefaultPageLoader status={"none"} />
                                ) : (
                                    tags?.map((tag) => (
                                                                    ...
                                                                    setFormData({
                                                                        ...
                                                                        executor_ids: isTeachersSelect
                                                                            ...
                                                                            : [
                                                                                ...
                                                                                ...teachers.map(item => ({
                                                                                    value: item.user_id,
                                                                                    label: `${item.name} ${item.surname} (${item.subject[0]?.name})`
                                                                                }))
                                                                            ]
                                                                    })
                                                                }}
                                                                type={isTeachersSelect ? "danger" : "success"}
                                                            >
                                                                O'qituvchilar
                                                            </Button>
                                                            <Button
                                                                onClick={() => {
                                                                    ...
                                                                    setFormData({
                                                                        ...
                                                                        executor_ids: isEpmloyeesSelect
                                                                            ? formData.executor_ids.filter(item =>
                                                                                !employers.map(e => e.user_id).includes(item.value)
                                                                            )
                                                                            : [
                                                                                ...formData.executor_ids,
                                                                                ...employers.map(item => ({
                                                                ...
                                                                type={isEpmloyeesSelect ? "danger" : "success"}
                                                            >
                                                                Ishchilar
                                                            </Button>
                                                        </>
                                ...
                                <div className={styles.formRow}>
                                    ...
                                    <div className={styles.formGroup}>
                                        ...
                                        <input
                                            ...
                                            type="number"
                                            min="1"
                                            value={formData.recurring_type !== "custom"
                                                ? recurringTypes.filter(item => item.id === formData.recurring_type)[0]?.number
                        ...
                        <div className={styles.modalLarge} onClick={(e) => e.stopPropagation()}>
                            ...
                            <div className={styles.viewContent}>
                                ...
                                <div className={styles.viewContent__tags}>
                                    ...
                                    <div className={classNames(styles.tagsContainer, { [styles.none]: selectedTask.tags.length === 0 })}>
                                        {selectedTask.tags.length === 0 ? (
                                            <h2>Teglar qo'shilmagan</h2>
                                        ) : (
                                            selectedTask.tags.map(item => (
                                                <h3 key={item.id} className={styles.tag}>{item.name}</h3>
                                            ))
                                        )}
                                    </div>
                                </div>
                                                ...
                                                {tasksProfileLoading && tasksProfileLoading === "subtasks" ? (
                                                    ...
                                                    [...selectedTask.subtasks]?.sort(compareByOrder)?.map((st) => (
                                                        <div key={st.id} className={styles.nestedItem}>
                                                            <p style={st.is_done ? { textDecoration: "line-through", color: "#6b7280" } : null}>
                                                                {st.order}. {st.title}
                                                            </p>
                                        ...
                                        <>
                                            <div className={styles.collapsibleContent}>
                                                {[...selectedTask.attachments]?.reverse().map((att) => (
                                                    <div key={att.id} className={classNames(styles.nestedItem, styles.dual)}>
                                                        <div className={styles.nestedItem__header}>
                                                            <div className={classNames(styles.nestedActions, styles.btns, styles.innerDual)}>
                                        ...
                                        <>
                                            <div className={styles.collapsibleContent}>
                                                {[...selectedTask.comments].reverse().map((com) => (
                                                    <div key={com.id} className={classNames(styles.nestedItem, styles.dual)}>
                                                        <div className={styles.nestedItem__header}>
                                                            <div className={styles.info}>
                                                        ...
                                                        {com.attachment && (
                                                            <div className={styles.nestedItem__content}>
                                                                <img crossOrigin="anonymous" src={com.attachment} alt="" />
                                                            </div>
                                                        )}
                                                    </div>
                                                ))}
                                        ...
                                        <>
                                            <div className={styles.collapsibleContent}>
                                                {[...selectedTask.proofs].reverse().map((proof) => (
                                                    <div key={proof.id} className={classNames(styles.nestedItem, styles.dual)}>
                                                        <div className={styles.nestedItem__header}>
                                                            <div className={classNames(styles.nestedActions, styles.btns, styles.innerDual)}>
                    ...
                    <div className={styles.modalBackdrop} onClick={() => setModalType(null)}>
                        <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
                            ...
                            <div className={styles.formActions}>
                                ...
                                <button className={styles.btnPrimary} onClick={handleChangeStatus}>
                                    Update
                                </button>
                            </div>
                        </div>
                ...
                {(modalType === "createTag" || modalType === "editTag") && (
                    <div className={styles.modalBackdrop} onClick={() => setModalType(null)}>
                        <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
                            <h2 className={styles.modalTitle}>
                                {modalType === "createTag" ? "Create Tag" : "Edit Tag"}
                            </h2>
                            <div className={styles.formGroup}>
                            ...
                            <div className={styles.formActions}>
                                ...
                                <button
                                    ...
                                    className={classNames(styles.btnPrimary, { [styles.disabled]: !tagFormData })}
            ...
            {/* Filter Modal */}
            <Modal active={isFilter} setActive={setIsFilter} extraClass={styles.filter}>
                <h1>Filter</h1>
                <div className={styles.filter__container}>
                    <Select
                        ...
                        onChangeOption={(value) => updateFilters("status", value)}
                    ...
                    <Select
                        ...
                        onChangeOption={(value) => updateFilters("category", value)}
                    ...
                    <div className={styles.dualInput}>
                        <Input
                            ...
                            title={"Deadline (from)"}
                            type={"date"}
                            onChange={(e) => updateFilters("deadlineFrom", e.target.value)}
                        ...
                        <Input
                            ...
                            title={"Deadline (to)"}
                            type={"date"}
                            onChange={(e) => updateFilters("deadlineTo", e.target.value)}
                    ...
                    <Button
                        ...
                        type={
                            filters.create || filters.deadlineFrom || filters.deadlineTo ||
                                filters.status !== "all" || filters.tags.length !== 0 || filters.category !== "all"
                                ? "danger"
                                : "disabled"
                        }
            ...
            {/* Redirect Modal */}
            <Modal setActive={setIsRedirected} active={isRedirected} extraClass={styles.redirect}>
                <h1>Redirect</h1>
                <Select
                    title={"Select redirect"}
                    options={teachersList.filter(item => item.id !== userId && item.id !== selectedTask?.executor?.id)}
