...
export const getEmpSalary  = createAsyncThunk(
    ...
    async ({branch, limit, offset , search}) => {
        ...
        return await request(`${API_URL}Users/salaries/?${ParamUrl({branch, limit, offset , search})}` , "GET" , null , headers())
    }
)
...
export const getDeletedEmpSalary  = createAsyncThunk(
    ...
    async ({branch, limit, offset , search}) => {
        const {request} = useHttp()
        return await request(`${API_URL}Users/salaries-deleted/?${ParamUrl({branch, limit, offset , search})}` , "GET" , null , headers())
    }
)
